...
palm.configure(api_key='AIzaSyBdYrFbOVdhQZLrEuwnk1FjmoCUWquHjxQ')
import requests
...
def index():
    if request.method == 'POST':
        ...
        symptoms_selected = request.form.to_dict(flat=False).keys()
        
        # Update symptoms dictionary based on user input
        for symptom in symptoms.keys():
            symptoms[symptom] = 1 if symptom in symptoms_selected else 0
...
def generate_text():
    ...
    try:
        ...
        model = models[0].name 
        generated_text = palm.generate_text(model=model, prompt=user_message, temperature=1.0, max_output_tokens=800)
